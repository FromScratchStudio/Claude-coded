...
export default function ProjectsView() {
  ...
  return (
    <div>
      <SectionTitle
        sub={`${projects.length} projects across ${rings.length} rings`}
        action={
      ...
      >
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: "0.85rem" }}>
          <div>
            <label style={labelStyle}>Ring</label>
        ...
        <div style={formRow}>
          <label style={labelStyle}>Progress: {pProgress}%</label>
          <input type="range" min={0} max={100} value={pProgress} onChange={(e) => setPProgress(Number(e.target.value))} style={{ width: "100%" }} />
        </div>
        <div style={formRow}>
          <label style={labelStyle}>Note</label>
          <input value={pNote} onChange={(e) => setPNote(e.target.value)} style={inputStyle} placeholder="Short description" />
        </div>
        <div style={formRow}>
          <label style={labelStyle}>Tags (comma-separated)</label>
          <input value={pTags} onChange={(e) => setPTags(e.target.value)} style={inputStyle} placeholder="design, product" />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: "0.85rem" }}>
          <div>
