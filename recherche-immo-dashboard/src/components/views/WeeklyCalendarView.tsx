...
export default function WeeklyCalendarView() {
  ...
  const [sPipelineItemId, setSPipelineItemId] = useState<string | null>(null);
  ...
  function openEditSlot(slot: ScheduleSlot) {
    ...
    setSPipelineItemId(slot.pipelineItemId ?? null);
    setSNote(slot.note ?? "");
  ...
  function save() {
    ...
    let slotPipelineItemId = sPipelineItemId;
    let slotNote = sNote;
    ...
    if (sSlotType === "unplanned") {
      ...
      if (!title) {
        alert("Le titre est obligatoire pour une tâche non planifiée.");
        return;
      }
      const startMinutes = parseTimeToMinutes(sStartTime);
      const endMinutes = parseTimeToMinutes(sEndTime);
      if (startMinutes === null || endMinutes === null || endMinutes <= startMinutes) {
        alert("Veuillez saisir une heure de début et de fin valides.");
        return;
      }
    ...
    if (editSlot) {
      updateScheduleSlot(editSlot.id, {
        ...
        pipelineItemId: slotPipelineItemId,
        note: slotNote,
        slotType: sSlotType,
        title: slotTitle,
  ...
  return (
    <div>
      <SectionTitle
        sub={`${utCount} / ${weeklyTarget} ${timeUnitLabel} planifiées · ${weekKey}`}
        action={
          <div style={{ display: "flex", gap: 6 }}>
            ...
            <button onClick={() => setWeekOffset(0)} style={{ background: C.surfaceAlt, border: `1px solid ${C.border}`, color: C.textMuted, borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: "0.75rem" }}>Aujourd’hui</button>
            <button onClick={() => setWeekOffset((o) => o + 1)} style={{ background: C.surfaceAlt, border: `1px solid ${C.border}`, color: C.textMuted, borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: "0.82rem" }}>&#8250;</button>
            <button
              onClick={() => { if (confirm("Effacer tous les créneaux de la semaine ?")) clearWeekSlots(weekKey); }}
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: "1.25rem", flexWrap: "wrap" }}>
        ...
        <span style={{ marginLeft: 8, fontSize: "0.78rem", color: C.textMuted, alignSelf: "center" }}>{utCount} {timeUnitLabel} remplies</span>
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: "1rem", flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: "0.72rem" }}>
          ...
          <span style={{ color: C.textMuted }}>Tâche non planifiée</span>
        </div>
        {workModes.map((mode) => (
      ...
      {unplannedSlots.length > 0 && (
        <div style={{ marginTop: "1rem", border: `1px solid ${C.border}`, borderRadius: 8, background: C.surface, padding: "0.9rem" }}>
          <div style={{ fontSize: "0.72rem", color: C.textDim, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "0.6rem" }}>
            Tâches non planifiées ({unplannedSlots.length})
          </div>
            ...
            {unplannedSlots.map((slot) => (
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", gap: 8, marginBottom: slot.description ? 4 : 0 }}>
                  <span style={{ color: UNPLANNED_COLOR, fontSize: "0.8rem", fontWeight: 600 }}>{slot.title || "Tâche non planifiée"}</span>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editSlot ? "Modifier créneau" : "Nouveau créneau"}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: "0.85rem" }}>
          <div>
            <label style={labelStyle}>Type de créneau</label>
        ...
        {sSlotType === "planned" ? (
          <>
            <div style={formRow}>
              <label style={labelStyle}>Durée : {fmtDuration(sDuration)}</label>
            ...
            <div style={formRow}>
              ...
              <input value={sNote} onChange={(e) => setSNote(e.target.value)} style={inputStyle} placeholder="Description du créneau" autoFocus />
            </div>
            <div style={formRow}>
              <label style={labelStyle}>Item pipeline lié</label>
              <select
                value={sPipelineItemId ?? ""}
                onChange={(e) => {
              ...
              >
                <option value="">Aucun</option>
                {pipelineItems.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}
              </select>
            </div>
            <div style={formRow}>
              <label style={labelStyle}>Complétion effective : {sEffectivePct}%</label>
              <input
                type="range"
                min={0}
                max={100}
          ...
          <>
            <div style={formRow}>
              ...
              <input value={sTitle} onChange={(e) => setSTitle(e.target.value)} style={inputStyle} placeholder="Titre de la tâche non planifiée" autoFocus />
            </div>
            <div style={formRow}>
              <label style={labelStyle}>Description</label>
              <input value={sDescription} onChange={(e) => setSDescription(e.target.value)} style={inputStyle} placeholder="Contexte optionnel" />
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: "0.85rem" }}>
              <div>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "space-between" }}>
          {editSlot && <button onClick={() => { removeScheduleSlot(editSlot.id); setShowModal(false); }} style={btnDanger}>Supprimer</button>}
